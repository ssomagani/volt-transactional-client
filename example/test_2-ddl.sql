...
create procedure test_1_proc 
	...
	as 
	update test_1 set name = ? where id = ?;
...
create procedure get_undo_test_1 
	partition on table undo_test_1 column txn_id 
	as 
	select undo_proc, name, id from undo_test_1 where txn_id = ? order by creation_time asc;
